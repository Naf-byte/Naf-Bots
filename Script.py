...
# Try to import Iterable from collections.abc
try:
...
def user_input(user_question):
    ...
    if conversation is None:
        st.error("Conversation chain not initialized!! Please upload PDF files first by clicking the arrow button on the top left corner.")
        return
    ...
    with st.spinner("Generating response..."):
        ...
        for i, message in enumerate(st.session_state.chatHistory):
            if i % 2 == 0:
                st.markdown(f"<div style='background-color: #32CD32; color: black; padding: 10px; border-radius: 10px; margin: 10px;'> [Me]🤖 :  {message.content} </div>", unsafe_allow_html=True)
            else:
                user_response = message.content
                st.markdown(f"<div style='background-color: #00008B; color: white; padding: 10px; border-radius: 10px; margin: 10px;'> [You]👱 :  {user_response} </div>", unsafe_allow_html=True)
